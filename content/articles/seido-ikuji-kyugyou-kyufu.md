---
{
  "id": "seido-ikuji-kyugyou-kyufu",
  "title": "育児休業給付金は給料の何%？180日を境に変わる給付率",
  "summary": "育児休業給付金は、休業開始から180日目までは休業前賃金の67%、それ以降は50%が目安。雇用保険の被保険者が対象。",
  "type": "seido-kaisetsu",
  "category": "仕事・キャリア",
  "tool_ref": null,
  "persona": "育休取得を検討する会社員",
  "revision_year": 2026,
  "sources": [
    { "url": "https://www.hellowork.mhlw.go.jp/insurance/insurance_continue.html", "title": "育児休業給付について", "org": "ハローワークインターネットサービス（厚生労働省）", "accessed": "2026-07-17" }
  ],
  "facts": [
    { "key": "ikuji_kyugyou.kyufu_180", "value": 67, "unit": "%", "seido_ref": "ikuji-kyugyou-kyufu.json#kyufu_ritsu.kaishi_kara_180nichi_made", "status": "verified" },
    { "key": "ikuji_kyugyou.kyufu_after", "value": 50, "unit": "%", "seido_ref": "ikuji-kyugyou-kyufu.json#kyufu_ritsu.181nichime_ikou", "status": "verified" },
    { "key": "ikuji_kyugyou.kirikae", "value": 180, "unit": "日", "seido_ref": "ikuji-kyugyou-kyufu.json#kirikae_nissu", "status": "verified" }
  ],
  "last_updated": "2026-07-17",
  "next_check_due": "2026-10-01"
}
---

# 育児休業給付金は給料の何%？180日を境に変わる給付率

## 結論

育児休業給付金は、育休開始から180日目を境に給付率が67%から50%に変わります。休業前の賃金をもとに計算されるため、実際の手取りの目安として覚えておくと安心です。

## 対象と金額

雇用保険に加入していて、一定の被保険者期間の条件を満たす人が対象です。

- 育休開始から180日目まで: 休業前賃金の67%
- それ以降: 休業前賃金の50%

いずれも上限額・下限額が別途定められているため、正確な金額は出典のシミュレーションでご確認ください。

## よくある疑問

- **給付率が変わるタイミングは？** 育休開始から180日目です。それより前と後で率が変わります。
- **パパも受け取れる？** 男女問わず、雇用保険の被保険者であれば対象です。

## 出典・根拠

- ハローワークインターネットサービス「育児休業給付について」

---

最終更新日: 2026-07-17 ／ 準拠年度: 2026年度 ／ 出典: 上記「出典・根拠」参照

※本記事は一次情報をもとにした情報整理です。制度の適用にあたっては、必ず最新の公式情報・窓口でご確認ください。
