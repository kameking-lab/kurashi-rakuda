...
describe("★★補足給付は試算対象月で切り替える★★（specs §4 ステップ6・§5.5・§6.4）", () => {
  ...
  it("T-34 / T-35 段階の境界（「以下」は含む・「超」は含まない）", () => {
    ...
    // 809,000ちょうど → 第2段階（「80.9万円以下」＝含む）
    expect(st(BOUNDARY_ADOPTED)).toBe("stage2");
  ...
  it("★★T-36 確定値を出さない★★ 80万円台前半は第2段階／第3段階①を併記する", () => {
    const r = shisetsu({ taxStatus: "hikazei", hojokyufuIncome: 820_000, roomType: "ユニット型個室" });
    // ★段階を確定しない★
    expect(r.stageResult.stage).toBeNull();
    expect(r.candidateLines).toHaveLength(2);
    expect(r.candidateLines.map((l) => l.stage.key)).toEqual(["stage2", "stage3a"]);
    // 両方の金額を出す（食費 390 と 650、居住費 880 と 1,370）
    expect(r.candidateLines[0].monthly).toBe((390 + 880) * 30);
    expect(r.candidateLines[1].monthly).toBe((650 + 1_370) * 30);

    const w = simulate(inp({ mode: "shisetsu", taxStatus: "hikazei", hojokyufuIncome: 820_000 })).warnings;
    expect(w.some((x) => x.includes("市区町村にご確認ください"))).toBe(true);
    expect(w.some((x) => x.includes("確定できません"))).toBe(true);
  });

  it("★不確定域はデータの note から導出する★（80.9万円 と 82.65万円 の食い違い）", () => {
    expect(BOUNDARY_ADOPTED).toBe(809_000);
    expect(BOUNDARY_ALTERNATIVE).toBe(826_500);
    // 採用値以下・対抗値超は確定できる。その間だけ確定しない
    expect(isStageAmbiguous(BOUNDARY_ADOPTED)).toBe(false);
    expect(isStageAmbiguous(BOUNDARY_ADOPTED + 1)).toBe(true);
    expect(isStageAmbiguous(BOUNDARY_ALTERNATIVE)).toBe(true);
    expect(isStageAmbiguous(BOUNDARY_ALTERNATIVE + 1)).toBe(false);
    // ★勝手に82.65万に寄せない★ データは告示本文の80.9万円を採用している
    expect(DATA.hojokyufu.userStages.stages.find((s) => s.key === "stage2")!.incomeMax).toBe(BOUNDARY_ADOPTED);
    expect(DATA.hojokyufu.userStages.boundaryDiscrepancy.value).toBe("未確認");
    expect(DATA.hojokyufu.userStages.boundaryDiscrepancy.note).toContain("確定値を出さず");
  });

  it("★罠16★ boundaryDiscrepancy.value は「未確認」という文字列（数値としてパースしない）", () => {
    expect(Number.isNaN(Number(DATA.hojokyufu.userStages.boundaryDiscrepancy.value))).toBe(true);
  });
