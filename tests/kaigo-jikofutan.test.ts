...
describe("高額介護サービス費（specs §4 ステップ5・§5.2・§6.3）", () => {
  ...
  it("★T-22改 罠9★ 非課税かつ年金等が境界ちょうど → 世帯24,600円／個人15,000円を併記（境界は80.9万→8/1から82.65万）", () => {
    ...
    const r = full({ careLevel: "yokaigo5", taxStatus: "hikazei", hojokyufuIncome: threshold });
    ...
    expect(
      full({ taxStatus: "hikazei", hojokyufuIncome: hikazei80manThreshold("2026-07-01") + 1 }).kougaku!.limitIndividual,
    ).toBeNull();
  });
