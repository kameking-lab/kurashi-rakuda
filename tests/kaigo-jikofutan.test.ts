...
describe("高額介護サービス費（specs §4 ステップ5・§5.2・§6.3）", () => {
  ...
  it("★★T-21 罠3★★ 世帯非課税は課税所得の入力なしで判定できる（線形探索に落とさない）", () => {
    const r = full({ careLevel: "yokaigo5", taxStatus: "hikazei", kougakuNenkinIncome: 1_500_000, taxableIncome: 0 });
  ...
  it("★T-22改 罠9★ 非課税かつ年金等が境界ちょうど → 世帯24,600円／個人15,000円を併記（境界は80.9万→8/1から82.65万）", () => {
    // ★2026-07-17 データ訂正★ 施行令の条文は80.9万円（「80万円」は誤りだった）。基準日で切替
    expect(hikazei80manThreshold("2026-07-31")).toBe(809_000);
    expect(hikazei80manThreshold("2026-08-01")).toBe(826_500);
    const threshold = hikazei80manThreshold("2026-07-01");
    const r = full({ careLevel: "yokaigo5", taxStatus: "hikazei", kougakuNenkinIncome: threshold });
    ...
    expect(
      full({ taxStatus: "hikazei", kougakuNenkinIncome: hikazei80manThreshold("2026-07-01") + 1 }).kougaku!.limitIndividual,
    ).toBeNull();
  });

  it("★遺族年金受給者の分離判定★ 高額介護（非課税年金を含めない）と補足給付（含める）で別の入力を使う", () => {
    // 例: 遺族年金150万円＋課税年金等70万円の単身・非課税世帯（2026-07-01時点）
    // 高額介護: 公的年金等収入＋その他所得 = 70万円 ≤ 80.9万円 → hikazei-80man（個人15,000円）
    // 補足給付: 非課税年金を含む 220万円 > 120万円 → 第3段階②
    const r = simulate(
      inp({
        mode: "shisetsu",
        careLevel: "yokaigo5",
        taxStatus: "hikazei",
        kougakuNenkinIncome: 700_000,
        hojokyufuIncome: 2_200_000,
      }),
    );
    expect(r.kougaku!.bracket.key).toBe("hikazei-80man");
    expect(r.kougaku!.limitIndividual).toBe(15_000);
    expect(r.hojokyufu!.stageResult.stage?.key).toBe("stage3b");
    // ★従前の単一入力では、どちらかの判定が必ず崩れていた★
    // （220万円で高額介護を判定すると hikazei=24,600円に、70万円で補足給付を判定すると第2段階に落ちる）
  });
  ...
  it("★機械判定しない区分★ genkaku-15000 / genkaku-24600 は自動選択されない（§7）", () => {
    ...
    for (const status of ["taxed", "hikazei", "seikatsuHogo"] as const) {
      for (const x of [0, 3_799_999, 3_800_000, 6_900_000, 9_999_999]) {
        const k = selectKougakuBracket(inp({ taxStatus: status, taxableIncome: x, hojokyufuIncome: x, kougakuNenkinIncome: x })).key;
...
describe("★制度の数値をコードに書かない★（data/seido から読んでいること）", () => {
  ...
  it("データを差し替えれば追随する（しきい値・限度額・施行日はすべて JSON 由来）", () => {
    ...
    expect(hikazei80manThreshold("2026-07-01")).toBe(
      DATA.kougakuKaigoServiceHi.brackets.find((b) => b.key === "hikazei-80man")!.incomeMaxBeforeAug2026,
    );
