...
describe("制度定数はすべて data/seido から読む（specs §4.0）", () => {
  ...
  it("★区分②の算式は未確認であるとデータが明記している（実装しない根拠）", () => {
    expect(J.calculationRules.rules[1].note).toContain("★未確認★");
...
describe("支給額の算定（specs §6.1・区分①・区分③・限度額）", () => {
  ...
  it("★最重要★ TC-04 境界B1の外（300,000 / 270,001）は区分② → 金額を出さない", () => {
    const b = calcBenefit(300_000, 270_001);
    expect(b.kubun).toBe("rule2");
    expect(b.kind).toBe("undeterminable");
    // ★10%を掛けて 27,000円 と出したら不合格★
    expect(b.amount).toBeNull();
    expect(b.reason).toContain("ハローワーク");
    expect(b.reason).not.toContain("27,000");
  });
...
describe("制度要件・各月要件（specs §6.2）", () => {
  ...
  it("★誤解の打ち消し★ TC-17 5%短縮（40h→38h）は区分②で算定不能だが「対象外」ではない", () => {
