...
describe("支給額の算定（specs §6.1・区分①・区分③・限度額）", () => {
  ...
  it("★回帰テストの錨★ TC-02 483,300 / 430,000 → 41,393円（calculationRules.example と完全一致）", () => {
    const b = calcBenefit(483_300, 430_000);
    expect(b.kubun).toBe("rule1");
    expect(b.amount).toBe(41_393);
    expect(b.capped).toBe(true);
    // データ側の example に記載された数値と突き合わせる
    expect(J.calculationRules.example).toContain("41,393円");
    expect(J.calculationRules.example).toContain("434,970");
    expect(benefitRule1(430_000)).toBe(43_000);
    expect(SUPPORT_LIMIT - 430_000).toBe(41_393);
  });
  ...
  it("★解禁の錨★ 区分② 厚労省パンフレット例②: 300,000 / 280,000 → 支給率6.43%・18,004円", () => {
    // データ側の exampleAdjustedRate と完全一致させる（率・金額とも）
    const b = calcBenefit(300_000, 280_000);
    expect(b.kubun).toBe("rule2");
    expect(b.kind).toBe("paid");
    expect(b.adjustedRatePercent).toBe(6.43);
    expect(b.amount).toBe(18_004);
    expect(J.calculationRules.exampleAdjustedRate).toContain("6.43%");
    expect(J.calculationRules.exampleAdjustedRate).toContain("18,004円");
  });
  ...
  it("TC-06 境界B3の内側: 500,000 / 428,530 → 合計471,383 ≦ 限度額なので満額 42,853円", () => {
    const b = calcBenefit(500_000, 428_530);
    expect(b.capped).toBe(false);
    expect(b.amount).toBe(42_853);
    expect(42_853 + 428_530).toBeLessThanOrEqual(SUPPORT_LIMIT);
  });

  it("TC-07 境界B3の外側: 500,000 / 428,540 → 頭打ちで 42,853円（TC-06と同額＝連続）", () => {
    const b = calcBenefit(500_000, 428_540);
    expect(b.capped).toBe(true);
    expect(b.amount).toBe(42_853);
    expect(b.amount).toBe(calcBenefit(500_000, 428_530).amount);
  });

  it("TC-08 境界B4: 賃金が支給限度額ちょうど（600,000 / 471,393）→ 不支給（B3→B5から自動的に0）", () => {
    const b = calcBenefit(600_000, SUPPORT_LIMIT);
    expect(benefitRule1(SUPPORT_LIMIT)).toBe(47_139); // 471,393 × 10% = 47,139.3 → 切捨
    expect(b.kind).toBe("notPaid");
    expect(b.amount).toBe(0);
  });

  it("TC-09 B4の1円手前（600,000 / 471,392）も最低限度額で消える", () => {
    const b = calcBenefit(600_000, SUPPORT_LIMIT - 1);
    // 算定額は 471,393 − 471,392 = 1円 → 2,411円以下 → 不支給
    expect(b.kind).toBe("notPaid");
    expect(b.amount).toBe(0);
  });

  it("TC-10 高所得: 600,000 / 450,000 → 10%（45,000）に届かず 21,393円（半額以下）", () => {
    const b = calcBenefit(600_000, 450_000);
    expect(benefitRule1(450_000)).toBe(45_000);
    expect(b.amount).toBe(21_393);
    expect(b.capped).toBe(true);
    expect(b.amount!).toBeLessThan(45_000 / 2);
  });

  it("★★浮動小数の罠★★ TC-11 境界B5: 40,000 / 24,110 → 算定額はちょうど 2,411円 → 不支給", () => {
    ...
    expect(benefitRule1(24_110)).toBe(2_411);
    for (const w of [24_110, 24_120, 266_666, 430_000, 471_393]) {
      expect(benefitRule1(w), `${w} で整数演算と一致すること`).toBe(Math.floor((w * 100) / 1000));
    }
    const b = calcBenefit(40_000, 24_110);
    expect(b.kind).toBe("notPaid"); // ★「以下」＝ちょうどは不支給側★ `<` で実装すると 2,411円 と出て不合格
    expect(b.amount).toBe(0);
    expect(benefitRule1(24_110)).toBe(MINIMUM_AMOUNT); // 境界そのもの
  });

  it("TC-12 B5の反対側: 40,000 / 24,120 → 2,412円で支給（1円差で分かれる）", () => {
    expect(benefitRule1(24_120)).toBe(2_412);
    const b = calcBenefit(40_000, 24_120);
    expect(b.kind).toBe("paid");
    expect(b.amount).toBe(2_412);
  });
...
describe("期限・異常系（specs §6.4）", () => {
  it("TC-32 境界B8の内側: 2026-07-31 は通常どおり計算する", () => {
    const r = simulate(inp({ today: "2026-07-31" }));
    expect(r.expired).toBe(false);
    expect(r.benefit.amount).toBe(24_000);
  });

  it("★境界B8★ TC-33 2026-08-01 は計算を停止する（古い金額で計算し続けない）", () => {
  ...
  it("TC-34 開始日が期限後（2026-09-01）でも計算を停止する", () => {
    const r = simulate(inp({ jitanStartDate: "2026-09-01", today: "2026-07-17" }));
    expect(r.expired).toBe(true);
    expect(r.benefit.amount).toBeNull();
    expect(isExpired({ today: "2026-07-17", jitanStartDate: "2026-09-01" })).toBe(true);
  });
...
describe("★8月1日のデータ差し替えだけで追随すること★（specs §7.1・§9.4）", () => {
  ...
  it("改定日はコードではなく amendments から導出される（expiresOn=2026-07-31 → 2026-08-01）", () => {
    ...
    expect(a!.summary).toContain("471,393");
    expect(a!.summary).toContain("2,411");
  });

  it("★安全弁★ expiresOn を1年後にすると期限切れが解消し、次回改定が2027-08-01になる", () => {
  ...
  it("★金額だけ差し替えても同じ式で追随する★ 限度額・最低限度額はデータ参照", () => {
    ...
    // 現行データでは 471,393 − 430,000 = 41,393
    expect(calcBenefit(483_300, 430_000).amount).toBe(SUPPORT_LIMIT - 430_000);
    // 頭打ちの発動点も限度額から導出される（コードに閾値を持たない）
    expect(calcBenefit(600_000, SUPPORT_LIMIT).amount).toBe(0);
