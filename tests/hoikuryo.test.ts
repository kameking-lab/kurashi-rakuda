...
describe("★大阪市の 2026-09-01★ 以降は tiers を参照せず一律0円", () => {
  ...
  it("大阪市・呉市以外には全員無償化の予定が無い", () => {
    for (const m of municipalities.filter((x) => !["osaka-osaka", "hiroshima-kure"].includes(x.id))) {
...
describe("★中核市バッチ6・最終★ 福井・明石・八尾・寝屋川（中部・近畿担当19市、完了）", () => {
  it("★福井市★ 境界値どおり階層が変わる（C1→C2の境界24,300円）", () => {
    expect(feeOfCalc({ municipalityId: "fukui-fukui", income: 24299 })).toBe(9100); // C1
    expect(feeOfCalc({ municipalityId: "fukui-fukui", income: 24300 })).toBe(13100); // C2
    expect(feeOfCalc({ municipalityId: "fukui-fukui", income: 397000 })).toBe(54900); // D9(最上位)
  });

  it("★明石市★ 境界値どおり階層が変わる（第3→第4の境界1円）", () => {
    expect(feeOfCalc({ municipalityId: "hyogo-akashi", income: 0, taxStatus: "equalOnly" })).toBe(9000); // 第3
    expect(feeOfCalc({ municipalityId: "hyogo-akashi", income: 1 })).toBe(11900); // 第4
    expect(feeOfCalc({ municipalityId: "hyogo-akashi", income: 397000 })).toBe(62500); // 第18(最上位)
  });

  it("★八尾市★ age2（2歳児クラス）は市独自施策で無償、0・1歳児クラスのみ課税", () => {
    const m = getMunicipality("osaka-yao")!;
    expect(isAge2Free(m)).toBe(true);
    expect(feeOfCalc({ municipalityId: "osaka-yao", age: "age2", income: 397000 })).toBe(0);
    expect(feeOfCalc({ municipalityId: "osaka-yao", income: 48599 })).toBe(11700); // C2
    expect(feeOfCalc({ municipalityId: "osaka-yao", income: 350000 })).toBe(63000); // D6(最上位)
  });

  it("★寝屋川市★ 23階層の細かい刻みでも境界値どおり階層が変わる", () => {
    expect(feeOfCalc({ municipalityId: "osaka-neyagawa", income: 19999 })).toBe(8400); // C1
    expect(feeOfCalc({ municipalityId: "osaka-neyagawa", income: 20000 })).toBe(10700); // C2
    expect(feeOfCalc({ municipalityId: "osaka-neyagawa", income: 397000 })).toBe(62600); // C21(最上位)
  });

  it("4市とも出典・免責・municipalityType=中核市を持つ", () => {
    for (const id of ["fukui-fukui", "hyogo-akashi", "osaka-yao", "osaka-neyagawa"]) {
      const m = getMunicipality(id)!;
      expect(m.sources.length, id).toBeGreaterThan(0);
      expect(m.disclaimer.length, id).toBeGreaterThan(0);
      expect(m.municipalityType, id).toBe("中核市");
...
describe("★年齢クラス拡張 age2（青森市ブロッカー解消・後方互換）★", () => {
  ...
  it("age2未使用の自治体では従来挙動を壊さない（後方互換。八尾市はage2実使用のため除外）", () => {
    for (const m of municipalities.filter((x) => x.id !== "osaka-yao")) {
