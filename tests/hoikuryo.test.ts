...
describe("★年齢クラス拡張 age2（青森市ブロッカー解消・後方互換）★", () => {
  ...
  it("age2 を持たない自治体は従来挙動を壊さない（後方互換・大多数）", () => {
    // ★age2 は青森市・八尾市など2歳児クラスを独自無償化する自治体でのみ使う。
    // それ以外の自治体（大多数）は age2 非設定＝under3 が0〜2歳を一律に扱う従来動作であることを固定する。
    const nonAge2 = municipalities.filter(
      (m) => !isAge2Free(m) && !(m.ageClasses ?? []).some((a) => a.key === "age2"),
    );
    // 少なくとも既存の大多数が age2 非設定である（1〜2市だけが age2 を使う想定）
    expect(nonAge2.length).toBeGreaterThan(municipalities.length - 5);
    for (const m of nonAge2) {
      for (const t of m.tiers) {
        // age2 の fees は無い＝feeOf は null（誤った数値を返さない）
        expect(feeOf(t, "age2", "standard"), `${m.id} ${t.tier} age2`).toBeNull();
      }
      // age2 を問い合わせても age2Free 無償にはならない（誤って0円にしない）
      const r = calc({
        municipalityId: m.id,
        month: "2026-06",
        age: "age2",
  ...
  it("age2Free を持つ自治体（八尾市など）は age2 で0円を返す（新機構が実データで動作）", () => {
    const age2Free = municipalities.filter((m) => isAge2Free(m));
    // 収集が進むと0件のこともあるため、存在する場合のみ挙動を固定する
    for (const m of age2Free) {
      expect((m.ageClasses ?? []).some((a) => a.key === "age2"), `${m.id} は ageClasses に age2`).toBe(true);
      const r = calc({
        municipalityId: m.id,
        month: "2026-06",
        age: "age2",
      ...
      expect(r?.fee, `${m.id} age2 fee`).toBe(0);
      expect(r?.basis, `${m.id} age2 basis`).toBe("age2Free");
      // 0・1歳児クラス（under3）は従来どおり課税判定へ進む（age2Free で0円にしない）
      const r01 = calc({
        municipalityId: m.id,
        month: "2026-06",
        age: "under3",
      ...
      expect(r01?.basis, `${m.id} under3 basis`).not.toBe("age2Free");
    }
  });
