...
describe("収集済み自治体（17件）", () => {
  it("17自治体を収録しており、id はファイル名と一致する", () => {
    expect(municipalities).toHaveLength(17);
...
describe("★名古屋市★ 年度が違うデータ（fiscalYear: 2025）", () => {
  ...
  it("名古屋以外の16自治体は令和8年度（2026-06）で年度不一致にならない", () => {
...
describe("T-28: 上級者モードには前処理の説明が必ず添えられる（データから取れる）", () => {
  ...
  it("前処理の説明・税年度ルールが17自治体すべてに存在する", () => {
    for (const m of municipalities) {
