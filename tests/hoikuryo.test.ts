...
describe("収集済み自治体（53件）", () => {
  it("53自治体を収録しており、id はファイル名と一致する", () => {
    expect(municipalities).toHaveLength(53);
...
describe("★京都市★ 保育利用時間バンド（timeBands・6段階）", () => {
  ...
  it("★他52自治体への影響なし★ timeBands を持つのは京都市のみで、バンド指定は金額を変えない", () => {
    const others = municipalities.filter((x) => x.id !== "kyoto-kyoto");
    expect(others).toHaveLength(52);
