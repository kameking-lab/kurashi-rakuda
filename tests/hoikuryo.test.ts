...
describe("収集済み自治体（12件）", () => {
  it("12自治体を収録しており、id はファイル名と一致する", () => {
  ...
  it("T-25: 未収集自治体（さいたま市等）は選択肢に存在しない＝金額を出さない", () => {
    expect(getMunicipality("saitama-saitama")).toBeUndefined();
    expect(calc(input({ municipalityId: "saitama-saitama" }))).toBeNull();
  });
...
describe("★名古屋市★ 年度が違うデータ（fiscalYear: 2025）", () => {
  ...
  it("名古屋以外の11自治体は令和8年度（2026-06）で年度不一致にならない", () => {
...
describe("T-28: 上級者モードには前処理の説明が必ず添えられる（データから取れる）", () => {
  ...
  it("前処理の説明・税年度ルールが12自治体すべてに存在する", () => {
    for (const m of municipalities) {
