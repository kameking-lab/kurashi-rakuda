...
describe("収集済み自治体（95件）", () => {
  it("95自治体を収録しており、id はファイル名と一致する", () => {
    expect(municipalities).toHaveLength(95);
...
describe("★京都市★ 保育利用時間バンド（timeBands・6段階）", () => {
  ...
  it("★他93自治体への影響なし★ timeBands を持つのは京都市・岡崎市のみで、バンド指定は金額を変えない", () => {
...
describe("★全国展開・西日本バッチ1★", () => {
  it("8市の階層数と代表境界を保持する", () => {
    const expected = {
      ...
      "hyogo-takarazuka": 14,
      "hyogo-itami": 15,
      "tokushima-tokushima": 12,
      "saga-saga": 16,
      "hiroshima-higashihiroshima": 17,
    } as const;
    for (const [id, count] of Object.entries(expected)) {
      expect(getMunicipality(id)?.tiers, id).toHaveLength(count);
    }
    expect(feeOfCalc({ municipalityId: "osaka-ibaraki", income: 397000 })).toBe(78000);
    expect(feeOfCalc({ municipalityId: "tokushima-tokushima", income: 168999 })).toBe(0);
    expect(feeOfCalc({ municipalityId: "saga-saga", income: 397000 })).toBe(66300);
    expect(feeOfCalc({ municipalityId: "hiroshima-higashihiroshima", income: 48600 })).toBe(19200);
  });

  it("R7据置4市をunder-review、他4市をR8として記録する", () => {
    for (const id of ["osaka-kishiwada", "hyogo-kakogawa", "hyogo-takarazuka", "hyogo-itami"]) {
      const m = getMunicipality(id)!;
      expect(m.fiscalYear, id).toBe(2025);
      expect(m.amendments?.some((a) => a.status === "under-review"), id).toBe(true);
    }
    for (const id of ["osaka-ibaraki", "tokushima-tokushima", "saga-saga", "hiroshima-higashihiroshima"]) {
      expect(getMunicipality(id)?.fiscalYear, id).toBe(2026);
    }
  });
});
