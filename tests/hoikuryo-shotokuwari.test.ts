...
describe("8a — 推計値としての振る舞い（BACKLOG Q3-09 の解禁条件）", () => {
  ...
  it("社会保険料が概算のときは、その旨の caveat が立つ", () => {
    const est = estimateShotokuwari({ ...input, socialInsurance: { kind: "estimate" } });
  ...
  it("社会保険料の概算は年収に比例し、自治体の公式計算例の対収入比の幅に収まる", () => {
    // 大阪市の例は 10.0%、名古屋市の例は 15.0%。概算はこの間に入る
    const e = estimateSocialInsurance(5_000_000);
    expect(e / 5_000_000).toBeGreaterThan(0.1);
    expect(e / 5_000_000).toBeLessThan(0.15);
  });
