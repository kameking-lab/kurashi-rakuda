  ...
  "jitan-kyuyo": {
    ...
    formula: (
      <>
        ...
        <p>
          <strong>区分②の金額を表示していない理由</strong>
          ：この「調整後の支給率」の算式は厚生労働省令で定められています。法律（雇用保険法第61条の12第6項第2号）は
          「90分の割合を超える大きさの程度に応じ、100分の10から一定の割合で逓減するように厚生労働省令で定める率」と
          定めるのみで、本ツールはこの省令の該当条文を確認できていません。
          <strong>
            確認できていない算式で金額をお見せすると、実際より多い金額を表示してしまいます
          </strong>
          （逓減後の率は必ず10%より小さくなるためです）。そのため区分②では金額を表示せず、ハローワークへのご確認をご案内しています。
          分かっているのは「賃金額と支給額の合計が、時短を始める前の賃金月額を超えることはない」という趣旨のみです。
          <strong>区分②は制度の対象外という意味ではありません。</strong>
        </p>
        <p>
  ...
  "kaigo-jikofutan": {
    ...
    formula: (
      <>
        ...
        <p>
          <strong>年金収入等が80万円台前半の方へ（金額を確定できません）</strong>
          ：補足給付の第2段階と第3段階①の境界について、一次資料の間で数値が食い違っています。
          厚生労働省「介護保険最新情報Vol.1481」の告示本文は80.9万円と規定する一方、
          同じ文書の解説図は82.65万円を用いており、社会保障審議会介護給付費分科会の資料にも
          両方の数値が併存しています。直接の根拠となる介護保険法施行規則の現行条文を確認できていないため、
          <strong>本ツールはこの範囲だけ確定値を出さず、両方の段階の金額を併記しています</strong>
          。お住まいの市区町村にご確認ください。
        </p>
        <p>
