...
const implementations: Record<string, { formula: ReactNode }> = {
  ...
  "cosme-match": {
    formula: (
      <>
        <p>
          <strong>マッチングの仕組み</strong>：肌質×悩み×価格帯の条件に、選んだ条件すべてが合致する商品だけを機械的に列挙します（合致数のスコアリングや独自の順位付けはありません）。並び順は価格が安い順→五十音順の固定ルールで、編集部イチオシ・PR枠の仕組みはコードにありません。0件のときは正直に0件と表示します。
        </p>
        <p>
          <strong>成分×肌質の適合の根拠</strong>：「◯◯肌向け」の表示は、商品の全成分表示に含まれる成分を{" "}
          <code>data/cosme/ingredient-skin-map.json</code>{" "}
          （日本化粧品工業会の適正広告ガイドライン等、業界自主基準・公的機関の一次情報を出典とする成分×肌質の一般論表）と機械的に突き合わせて導出しています。個々の商品の「効果」を保証するものではなく、「この成分が配合されており、一般的にこの肌質の方に選ばれることが多い」という一般論です。
        </p>
        <p>
          <strong>公平性の担保</strong>：商品名はテキストのみで、メーカー公式サイトへのリンクを含め一切のリンクを設置していません（アフィリエイト・広告収益は一切発生しません）。全商品に「向かない人」「注意点」を必ず併記しており、これらが0件の商品はビルド時のチェック（check:cosme-fairness）でエラーになる仕組みです。
        </p>
        <p>
          <strong>YMYLの線引き</strong>
          ：成分の一般論は公的・業界自主基準の範囲にとどめ、個別商品の治療効果・医学的効能は謳いません。ニキビ・皮膚炎など疾患が疑われる場合は、結果より先に皮膚科への相談をおすすめします。
        </p>
        <p>初期収録は少数の代表的な商品からのスタートです。順次拡充していきます。</p>
      </>
    ),
  },
